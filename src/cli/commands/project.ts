...
export class ProjectListCommand implements Command {
  name = 'list';
  aliases = ['ls', 'l'];
...
export class ProjectOpenCommand implements Command {
  name = 'open';
  aliases = ['o'];
  ...
  private async indexProjectDocuments(
    project: any,
    documentService: any
  ): Promise<void> {
  ...
  async execute(args: string[], context: CLIContext): Promise<Result<CommandOutput, Error>> {
    ...
    try {
      ...
      if (result.value.metadata.settings.autoIndex) {
        ...
        this.indexProjectDocuments(
          result.value,
          context.services.documentService
        ).catch(error => {
...
export class ProjectCloseCommand implements Command {
  name = 'close';
  aliases = ['c'];
...
export class ProjectNewCommand implements Command {
  name = 'new';
  aliases = ['add', 'a'];
      ...
      if (!noIndex) {
        ...
        if (openResult.ok) {
          ...
          if (openResult.value.metadata.settings.autoIndex) {
            ...
            (openCommand as any).indexProjectDocuments(
              openResult.value,
              context.services.documentService
            ).then(() => {
...
export class ProjectRemoveCommand implements Command {
  name = 'remove';
  aliases = ['rm', 'delete'];
...
export class ProjectInfoCommand implements Command {
  name = 'info';
  aliases = ['i', 'details'];
  ...
  async execute(args: string[], context: CLIContext): Promise<Result<CommandOutput, Error>> {
    ...
    try {
      ...
      if (!dbResult.ok) {
        return Result.err(dbResult.error.error);
      }
      ...
      if (mlStatus) {
        ...
        if (mlStatus.models && mlStatus.models.length > 0) {
          message += `Models:\n`;
          mlStatus.models.forEach((model: any) => {
...
export class ProjectReindexCommand implements Command {
  name = 'reindex';
  aliases = ['re', 'refresh'];
  ...
  async execute(args: string[], context: CLIContext): Promise<Result<CommandOutput, Error>> {
    ...
    try {
      ...
      // Get database access
      const dbResult = (context.services.projectService as any).db.getDatabase();
      if (!dbResult.ok) {
      ...
      const existingDocs = db.prepare('SELECT id, path, content_hash FROM documents WHERE project_id = ?').all(project.id) as Array<{
        id: string;
        path: string;
        content_hash: string;
      }>;
      ...
      for (const pattern of indexPatterns) {
        ...
        const indexableFiles = files.filter(file => {
          ...
          return (openCommand as any).shouldIndexFile(file);
        });
