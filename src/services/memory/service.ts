import { Result } from '../../types/result';
import { 
  RequiredFieldError, 
  ServiceUnavailableError,
  NetworkError 
} from '../../types/errors';
import { getMLClient } from '../../dal/ml-client';
import { 
  IMemoryService, 
  MemoryServiceConfig, 
  SearchOptions, 
  SearchResult 
} from './types';
import { SearchResult as MLSearchResult } from '../../types/domain';

export class MemoryService implements IMemoryService {
  private readonly mlClient = getMLClient();

  constructor(_config?: MemoryServiceConfig) {
    // Initialize with optional config
  }

  async searchDocuments(query: string, options?: SearchOptions): Promise<Result<SearchResult[]>> {
    // Validate input
    if (!query || query.trim().length === 0) {
      return Result.err(new RequiredFieldError('query'));
    }

    try {
      // Forward to ML service search endpoint
      const mlResponse = await this.mlClient.search({
        query: query.trim(),
        limit: options?.limit ?? 10,
        searchType: options?.searchType ?? 'hybrid',
        projectIds: options?.projectId ? [options.projectId] : undefined
      });

      if (!mlResponse.ok) {
        // TODO: Add SQLite FTS fallback
        console.warn('ML search failed, falling back to FTS would happen here');
        return Result.ok([]); // Placeholder for FTS fallback
      }

      // Convert ML response to SearchResult format
      const results: SearchResult[] = mlResponse.value.results.map((r: MLSearchResult) => ({
        id: r.id,
        type: 'document' as const,
        score: r.score,
        content: r.content,
        preview: r.content.substring(0, 150) + '...',
        metadata: {
          projectId: r.projectId,
          filePath: r.path,
          title: r.title
        }
      }));

      return Result.ok(results);
    } catch (error) {
      return Result.err(new NetworkError('ML service search failed'));
    }
  }

  async searchThoughts(query: string, options?: SearchOptions): Promise<Result<SearchResult[]>> {
    // Validate input
    if (!query || query.trim().length === 0) {
      return Result.err(new RequiredFieldError('query'));
    }

    try {
      // Forward to ML service thought search
      const mlResponse = await this.mlClient.searchThoughts(
        query.trim(),
        options?.limit ?? 10
      );

      if (!mlResponse.ok) {
        // TODO: Add SQLite FTS fallback for thoughts
        console.warn('ML thought search failed, falling back to FTS would happen here');
        return Result.ok([]); // Placeholder for FTS fallback
      }

      // Convert ML response to SearchResult format
      const results: SearchResult[] = mlResponse.value.results.map((r: MLSearchResult) => ({
        id: r.id,
        type: 'thought' as const,
        score: r.score,
        content: r.content,
        preview: r.content.substring(0, 150) + '...',
        metadata: {
          projectId: r.projectId,
          title: r.title
        }
      }));

      return Result.ok(results);
    } catch (error) {
      return Result.err(new NetworkError('ML thought search failed'));
    }
  }

  async search(query: string, options?: SearchOptions): Promise<Result<SearchResult[]>> {
    // Validate input
    if (!query || query.trim().length === 0) {
      return Result.err(new RequiredFieldError('query'));
    }

    try {
      // Unified search across all content types
      // For now, just combine document and thought results
      const [docsResult, thoughtsResult] = await Promise.all([
        this.searchDocuments(query, { ...options, limit: (options?.limit ?? 10) / 2 }),
        this.searchThoughts(query, { ...options, limit: (options?.limit ?? 10) / 2 })
      ]);

      const allResults: SearchResult[] = [];

      if (docsResult.ok) {
        allResults.push(...docsResult.value);
      }

      if (thoughtsResult.ok) {
        allResults.push(...thoughtsResult.value);
      }

      // Sort by score (highest first) and apply limit
      allResults.sort((a, b) => b.score - a.score);
      const limitedResults = allResults.slice(0, options?.limit ?? 10);

      return Result.ok(limitedResults);
    } catch (error) {
      return Result.err(new ServiceUnavailableError('search', error as Error));
    }
  }

  // TODO: Future methods could include:
  // - searchByDateRange
  // - searchRelated (find related content)
  // - searchSimilar (find similar content)
  // - getSearchSuggestions
}